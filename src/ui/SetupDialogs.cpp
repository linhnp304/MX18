...
EngineerDialog::EngineerDialog(QWidget *parent)
    : QDialog(parent)
{
    setWindowTitle(QStringLiteral("Điều khiển kỹ sư"));
    setMinimumWidth(420);

    auto *lay = new QVBoxLayout(this);

    auto *pwGroup = new QGroupBox(QStringLiteral("Mật khẩu kỹ sư"), this);
    auto *pwLay = new QHBoxLayout(pwGroup);
    auto *pwEdit = new QLineEdit(Settings::instance().engineerPassword(), pwGroup);
    auto *pwBtn = new QPushButton(QStringLiteral("Đổi mật khẩu"), pwGroup);
    pwLay->addWidget(new QLabel(QStringLiteral("Mật khẩu:"), pwGroup));
    pwLay->addWidget(pwEdit, 1);
    pwLay->addWidget(pwBtn);
    lay->addWidget(pwGroup);

    connect(pwBtn, &QPushButton::clicked, this, [pwEdit, this] {
        const QString pw = pwEdit->text().trimmed();
        if (pw.isEmpty()) {
            QMessageBox::warning(this, QStringLiteral("Mật khẩu"),
                                 QStringLiteral("Mật khẩu không được để trống."));
            return;
        }
        Settings::instance().setEngineerPassword(pw);
        QMessageBox::information(this, QStringLiteral("Mật khẩu"),
                                 QStringLiteral("Đã lưu mật khẩu kỹ sư vào settings/params.json."));
    });

    auto *note = new QLabel(
        QStringLiteral("Các tham số mức kỹ sư (địa chỉ mạng, tham số đài, hiệu chỉnh)\n"
                       "sẽ bổ sung ở giai đoạn sau và lưu chung trong settings/params.json."),
        this);
    note->setStyleSheet(QStringLiteral("color:#5d666f;font-style:italic;"));
    lay->addWidget(note);
    lay->addStretch(1);
    ...
    box->button(QDialogButtonBox::Close)->setText(QStringLiteral("Đóng"));
    connect(box, &QDialogButtonBox::rejected, this, &QDialog::reject);
    lay->addWidget(box);
}
